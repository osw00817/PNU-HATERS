...
void shiftUp(int *arr, int childIdx)
{
    ...
    if(parentIdx>=0 && abs(arr[parentIdx])>abs(arr[childIdx]) ||
    (abs(arr[parentIdx]) == abs(arr[childIdx]) && arr[parentIdx] > arr[childIdx]))
    {
        temp = arr[parentIdx];
...
void shiftDown(int* arr, int parentIdx, int size)
{
    ...
    int temp;
if (leftIdx < size &&
    (abs(arr[leftIdx]) < abs(arr[smallIdx]) ||
    (abs(arr[leftIdx]) == abs(arr[smallIdx]) && arr[leftIdx] < arr[smallIdx]))) {
    smallIdx = leftIdx;
}

if (rightIdx < size &&
    (abs(arr[rightIdx]) < abs(arr[smallIdx]) ||
    (abs(arr[rightIdx]) == abs(arr[smallIdx]) && arr[rightIdx] < arr[smallIdx]))) {
    smallIdx = rightIdx;
}
